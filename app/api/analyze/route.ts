...
function normalizeUrl(raw: string) {
  ...
  if (h === 'localhost' || h === '127.0.0.1' || h === '::1' || h.endsWith('.local') || /^10\./.test(h) || /^192\.168\./.test(h) || /^172\.(1[6-9]|2\d|3[0-1])\./.test(h)) {
...
function hostKey(hostname: string) {
  ...
  return Object.keys(presets).find(k => host === k || host.endsWith(`.${k}`));
}

function companyFrom(hostname: string, title?: string) {
  if (title) {
...
function detect(text: string): Category[] {
  ...
  return taxonomy.map(t => {
    const hits = t.keys.reduce((n, key) => n + (lower.includes(key) ? 1 : 0), 0);
    return hits ? { name: t.name, applications: t.applications, confidence: Math.min(99, 70 + hits * 9) } : null;
...
export async function POST(req: NextRequest) {
  try {
    ...
    let source: 'website' | 'curated-fallback' = 'website';
    ...
    try {
      ...
      const res = await fetch(url.toString(), {
        ...
        redirect: 'follow',
        headers: { 'user-agent': 'Mozilla/5.0 ProjectSignal validation bot' },
        cache: 'no-store',
      });
      clearTimeout(timer);
      ...
      // Some enterprise sites block server-side fetches. Curated demo profiles keep validation flow usable.
    }

    if (categories.length < 2 && presetKey) {
      categories = presets[presetKey];
      source = 'curated-fallback';
    }
    if (!categories.length) {
      categories = [
        { name: 'Commercial building systems', applications: 'Commercial and public projects', confidence: 68 },
      ];
      source = 'curated-fallback';
    }
    ...
    return NextResponse.json({
      company: companyFrom(url.hostname, title),
      hostname: url.hostname.replace(/^www\./, ''),
      source,
      categories: categories.slice(0, 6),
      idealProjects: applications.slice(0, 8),
